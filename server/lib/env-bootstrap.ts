 ...
 *
 * アプリ設定値 (CERNERE_BASE_URL / AEDILIS_ADMIN_IDS 等) は Infisical に置く。
 * Infisical に到達するための machine identity (INFISICAL_*) だけはどこかから
 * 渡す必要がある:
 *
 *   (A) Excubitor/Concordia 経由: parent が child env に INFISICAL_* を inject
 ...
 *
 * Memoria / Cernere / Bibliotheca と同じパターン。
 */
...
const WANTED_KEYS: readonly string[] = [
  ...
  'AEDILIS_PUBLIC_URL',
  'AEDILIS_ADMIN_IDS',
  // Cernere 上の "aedilis" project 登録用資格情報。 Infisical には置かず、 Excubitor が
  // 起動のたびに Cernere へ発行させて子 env へ注入する (catalog の cernere_launch_credentials)。
  // injectSecrets は既存 process.env を上書きしないので、 Infisical に同名 secret が
  // 残っていても注入値が勝つ。 ここでは「揃っているか」の点検対象として並べる。
